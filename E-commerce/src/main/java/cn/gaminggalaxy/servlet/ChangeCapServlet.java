...
/**
 * Servlet implementation class ChangeCapServlet
 */
@WebServlet("/change-cap")
public class ChangeCapServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ChangeCapServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (PrintWriter out = response.getWriter()) {
			...
			String cap = request.getParameter("modifica-cap");

			User user = new User(productId, cap,0,0,0,0);
			try {
			    UserDao userDao = new UserDao(DbCon.getConnection());
			    boolean success = userDao.changeCap(user, request);
