...
/**
 * Servlet implementation class ChangeCittaServlet
 */
@WebServlet("/change-citta")
public class ChangeCittaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ChangeCittaServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (PrintWriter out = response.getWriter()) {
			...
			String citta = request.getParameter("modifica-citta");

			User user = new User(productId, citta,"unused",0,0);
			try {
			    UserDao userDao = new UserDao(DbCon.getConnection());
			    boolean success = userDao.changeCitta(user, request);
