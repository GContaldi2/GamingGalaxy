...
/**
 * Servlet implementation class ChangeCvvServlet
 */
@WebServlet("/change-cvv")
public class ChangeCvvServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ChangeCvvServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (PrintWriter out = response.getWriter()) {
			...
			String cvv = request.getParameter("modifica-cvv");

			User user = new User(productId,cvv,0,0,0,0,0,0,0);
			try {
			    UserDao userDao = new UserDao(DbCon.getConnection());
			    boolean success = userDao.changeCvv(user, request);
