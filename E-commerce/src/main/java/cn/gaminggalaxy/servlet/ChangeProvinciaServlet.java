...
/**
 * Servlet implementation class ChangeProvinciaServlet
 */
@WebServlet("/change-provincia")
public class ChangeProvinciaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ChangeProvinciaServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (PrintWriter out = response.getWriter()) {
			...
			String provincia = request.getParameter("modifica-provincia");

			User user = new User(productId,provincia,0,0,0);
			try {
			    UserDao userDao = new UserDao(DbCon.getConnection());
			    boolean success = userDao.changeProvincia(user, request);
