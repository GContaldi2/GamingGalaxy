...
public class ChangeAddressServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (PrintWriter out = response.getWriter()) {
			...
			String indirizzo = request.getParameter("modifica-indirizzo");

			User user = new User(productId, indirizzo,"unused",0);
			try {
			    UserDao userDao = new UserDao(DbCon.getConnection());
			    boolean success = userDao.changeAddress(user, request);
